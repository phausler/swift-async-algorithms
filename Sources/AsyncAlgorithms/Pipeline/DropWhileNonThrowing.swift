extension Pipeline {
  public consuming func dropWhile(
    _ predicate: nonisolated(nonsending) @escaping @Sendable (borrowing Element) async -> Bool
  ) -> some Pipeline<Element, Failure> & ~Copyable {
    DropWhile(self, predicate: predicate)
  }
}

fileprivate struct DropWhile<Base: Pipeline>: ~Copyable {
  var base: Base
  var predicate: (nonisolated(nonsending) @Sendable (borrowing Element) async -> Bool)?
  var finished = false

  init(_ base: Base, predicate: nonisolated(nonsending) @escaping @Sendable (borrowing Element) async -> Bool) {
    self.base = base
